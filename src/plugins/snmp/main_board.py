...
class MainBoardPlugin(BasePlugin):
    def linux(self):
        ...
        try:
            response.data = {'sn': None, 'fast_server_number': None, 'manufacturer': None, 'model': None}
            ...
            temp = self.exec_shell_cmd('snmpwalk -v 2c -c %s %s .1.3.6.1.4.1.674.10892.5.1.1.4.0' % (settings.community_name, self.manager_ip))
            manufacturer = temp.split('"')[-2]
            response.data['manufacturer'] = manufacturer
